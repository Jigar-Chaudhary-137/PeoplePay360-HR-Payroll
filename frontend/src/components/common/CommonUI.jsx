...
export function EmptyState({ icon: Icon, title, description, actionText, onAction }) {
  return (
    <div className="glass-card p-12 text-center flex flex-col items-center justify-center">
      ...
      <h4 className="text-lg font-bold text-[#171717]">{title}</h4>
      {description && <p className="text-sm text-[#6B7280] max-w-sm mt-1 mb-5">{description}</p>}
      {actionText && onAction && (
...
export function StatCard({ title, value, subtitle, icon: Icon, trend, color = 'purple' }) {
  const colorMap = {
    purple: 'bg-[#F3E8FF] text-[#6D28D9] border-[#E5E7EB]',
    sky: 'bg-blue-50 text-blue-600 border-blue-100',
  ...
  return (
    <div className="glass-card p-5 relative overflow-hidden group">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider text-[#6B7280] font-heading">{title}</p>
          <h3 className="text-2xl font-extrabold text-[#171717] mt-1 font-heading">{value}</h3>
          {subtitle && <p className="text-xs text-[#6B7280] mt-1">{subtitle}</p>}
        </div>
        {Icon && (
          <div className={`p-3 rounded-xl ${colorMap[color] || colorMap.purple} border transition-transform group-hover:scale-105`}>
            <Icon size={22} />
          </div>
        )}
      </div>
...
export function ErrorState({ message = 'Something went wrong.', onRetry }) {
  return (
    <div className="glass-card p-12 text-center flex flex-col items-center justify-center">
      ...
      <h4 className="text-lg font-bold text-[#171717]">Something went wrong</h4>
