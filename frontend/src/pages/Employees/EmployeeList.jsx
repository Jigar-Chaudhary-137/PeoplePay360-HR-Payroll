                ...
                {employees.map((emp) => (
                  <tr key={emp.id} className="group">
                    ...
                    <td>
                      ...
                      <div className="text-xs text-slate-400">{emp.department_name || 'General'}</div>
                    </td>
                    <td>
                      {emp.active_wage ? (
                        <div>
                          <div className="font-bold text-sky-400">₹{Number(emp.active_wage).toLocaleString()}</div>
                          <div className="text-xs text-slate-400">{emp.active_contract_code}</div>
                        </div>
                      ) : (
                    ...
                    <td className="text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <Link
                          to={`/employees/${emp.id}`}
                          className="p-2 rounded-lg hover:bg-white/10 text-slate-400 hover:text-white transition-colors min-h-[36px] flex items-center justify-center"
                        ...
                        {hasRole('HR Manager', 'Admin') && (
                          <button
                            ...
                            className="p-2 rounded-lg hover:bg-white/10 text-slate-400 hover:text-sky-400 transition-colors min-h-[36px] flex items-center justify-center"
                            title="Edit Employee"
                          >
                            <Edit size={16} />
                          </button>
                        ...
                        {hasRole('Admin') && (
                          <button
                            ...
                            className="p-2 rounded-lg hover:bg-rose-500/10 text-slate-400 hover:text-rose-400 transition-colors min-h-[36px] flex items-center justify-center"
                            title="Delete"
                          >
                            <Trash2 size={16} />
                          </button>
            ...
            return (
              <div key={dept.id} className="glass-card p-4 space-y-3">
                <div className="flex items-center justify-between border-b border-white/5 pb-2">
                  ...
                  <span className="text-xs px-2.5 py-0.5 rounded-full bg-white/5 text-slate-400 font-bold">
                    {deptEmps.length}
                  </span>
                </div>
                  ...
                  {deptEmps.length === 0 ? (
                    ...
                    deptEmps.map((emp) => (
                      <div
                        ...
                        className="p-3.5 rounded-xl bg-slate-900/90 border border-white/5 hover:border-sky-500/40 cursor-pointer transition-all space-y-2 group shadow-sm hover:shadow-sky-500/10"
                      >
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-bold text-sky-400">{emp.emp_code}</span>
                          <Badge status={emp.employment_status} />
                        </div>
                        <div>
                        ...
                        <div className="text-xs text-slate-300 font-semibold pt-1 border-t border-white/5 flex justify-between">
                          <span className="text-slate-400 text-xs">Wage:</span>
