...
export function EmployeePortal() {
  ...
  return (
    <div className="space-y-6 pb-8">
      {/* Welcome Banner */}
      <div className="card p-6 bg-gradient-to-r from-blue-50/80 via-white to-indigo-50/80 border-blue-200">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <span className="text-xs uppercase font-bold tracking-wider text-blue-700">Employee Self-Service</span>
            <h1 className="text-2xl xl:text-3xl font-bold text-slate-900 mt-1">
              Welcome, {user?.first_name} {user?.last_name}
            </h1>
            <p className="text-sm text-slate-600 mt-1">
              Code: <strong className="text-blue-700 font-mono">{user?.emp_code}</strong> • Role: <strong className="text-slate-800">{user?.role}</strong> • Department: <strong className="text-slate-800">{user?.department_name || 'General'}</strong>
            </p>
          </div>
        ...
        <div className="card p-6 space-y-4">
          ...
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-3">
            ...
            {todayStatus?.worked_hours > 0 && (
              <div className="flex items-center justify-between text-sm pt-2.5 border-t border-slate-200">
                <span className="text-slate-600 font-medium">Total Shift Hours:</span>
                <span className="font-bold text-blue-700 font-mono">{todayStatus.worked_hours} Hours</span>
              </div>
            )}
          </div>
          ...
          {latestPayslip ? (
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-emerald-50/60 border border-emerald-200 flex items-center justify-between">
                <div>
                  <span className="text-xs text-slate-500 font-semibold block">Period: {latestPayslip.period_month}</span>
                  <span className="text-2xl font-black text-emerald-700">
                    ₹{Number(latestPayslip.net_salary).toLocaleString()}
                  </span>
                </div>
                <div className="text-right text-xs text-slate-600 space-y-0.5">
                  <p>Gross: <strong className="text-slate-800">₹{Number(latestPayslip.gross_salary).toLocaleString()}</strong></p>
                  <p className="text-rose-600">Deductions: <strong className="text-rose-700">₹{Number(latestPayslip.total_deductions).toLocaleString()}</strong></p>
                </div>
              </div>
            ...
            <p className="text-sm text-slate-500 py-8 text-center">No finalized payslips available yet.</p>
          )}
        </div>
      </div>
      ...
      <div className="space-y-3.5">
        <h3 className="font-bold text-slate-900 text-base">My Leave Balances (2026)</h3>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {allocations.map((al) => (
            <div key={al.id} className="card p-5 space-y-2.5 border-l-4" style={{ borderColor: al.type_color || '#2563eb' }}>
              <div className="flex items-center justify-between">
                <p className="text-xs font-bold text-slate-700 uppercase tracking-wider">{al.type_name}</p>
                <span className="text-xs text-slate-500 font-medium">Total: {al.allocated_days}d</span>
              </div>
              <div className="flex items-baseline gap-2">
                <span className="text-2xl font-bold text-slate-900">{al.remaining_days}</span>
              ...
              <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    ...
                    backgroundColor: al.type_color || '#2563eb'
                  }}
                />
              </div>
            </div>
